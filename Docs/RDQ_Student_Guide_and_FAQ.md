# 🛡️ RDQ 水循環學習戰隊 ｜ 學生專屬實戰操作手冊 & FAQ 終極指南 (AI 自動判題完全體版)
...
5. 🌐 **第五部分：Web 儀表板 AI 自動判題與閃卡操作 (localhost:8000)**
6. 📸 **第六部分：考卷錯題上傳與 Pending 緩衝區 (2 大方法)**
...
## 🌐 第五部分：Web 儀表板 AI 自動判題與閃卡操作 (http://localhost:8000)
...
|  +-----------------------------------------------------------------+  |
|  [ 提交答案 ➔ 由 AI 判定驗證 ]                                        |
|  🟢 判定正確 ➔ 亮綠燈 ➔ 自動晉級 ➔ 閃卡滑出消失！                           |
|  🔴 判定不精準 ➔ 亮紅燈顯示解析提示 ➔ 3.5秒後自動降級保護                   |
+-----------------------------------------------------------------------+
```

### 🤖 升級亮點：AI 自動判定，徹底消除「學生自評」與「重複答題 Bug」
- **完全由 AI 自動審查**：徹底移除了自評按鈕！學生輸入答案點擊「提交答案」後，系統透過 `POST /api/verify` 端點由 AI 自動比對文字觀念或數字。
- **即時正向回饋與消去**：
  - **判定正確** ➔ 亮起綠燈 `✅ 答對了！` ➔ 自動晉級 ➔ 閃卡播放平滑滑出動畫**立刻從清單中消失**，右上角「已成功晉級」計數器 +1！
  - **跨午夜防重複**：今日已防禦完成的題目，跨過午夜 12 點前絕不重複出現強迫重答。

---

## 📸 第六部分：考卷錯題上傳與 Pending 緩衝區 (2 大方法)

### 方法 A：對話框拍照上傳 (最推薦)
1. 用手機拍下考卷或小考錯題。
2. 上傳至對話框並說明：*「把這張考卷照片加入錯題本」*。
3. AI 自動辨識題目與解析，並寫入資料庫！若遇到解析置信度較低的圖片，會自動進入 **Pending 緩衝區 (`ingestion_staging`)** 待確認，阻絕錯誤資料污染。
...
### Q1：閃卡是由學生自己按對錯，還是由 AI 判定？
**答：已全面升級由 AI 自動判定對錯！** 徹底移除了學生自評按鈕。孩子在實體阻力區輸入答案後，系統呼叫 `POST /api/verify` 由 AI 自動比對，答對自動晉級並滑出，答錯亮紅燈顯示解析並於 3.5 秒後降級。

### Q2：答對晉級後的題目會不會重複出現在下面讓我重答？
**答：絕對不會！** 系統已實裝 `updated_at` 午夜時間過濾，答對或防禦完成的閃卡會立刻播放滑出動畫並從清單消失，午夜 12 點前絕不重複出現。

### Q3：課本上傳後也會符合 RDQ 複習模式嗎？
**答：100% 完全符合，而且精準度更高！** 蘇格拉底反問、L1/L2 鷹架降級、迷思澄清與 Leitner 記憶曲線完全照常運作，差別僅在於知識邊界被 100% 鎖定在您上傳的這份課文中。

### Q4： Phase 4 盲點提示的問題是怎麼來的？會不會隨便出題？
**答：來自歷屆會考大數據與心測中心高誘答率題庫！** 系統解析了 `mapped_exams.json`，將會考最高頻、最容易掉坑的陷阱題轉譯為生活化對話問句，並具備 `mc_probe_variant` 變體排除機制，確保絕不重複死板出題。

### Q5：如果孩子說得太簡短，系統會不會亂問超綱題目？
**答：絕對不會！** 系統有防呆機制。API 已經寫入 `scope_disputed != 1` 與 `grade` 學期過濾，非當前進度的題目絕不出現在眼前。

### Q6：如果孩子以為自己懂了，但觀念是錯的，系統會發現嗎？
**答：會的！** 這正是「蘇格拉底反問」的威力。當孩子自信給出錯誤答案時，AI 會以非評判語氣引導：「很多人也這樣想喔！不過其實…」，並用簡單的微型驗證確定孩子真正釐清觀念。

### Q7：實體阻力 (Physical Friction) 一定要在 Web 端打字嗎？
**答：是的！** 直接翻牌看答案只會產生「流暢性幻覺」（看過以為會了）。在解鎖前打出 3~5 個關鍵字，是大腦神經元活化並形成長期記憶的關鍵。

### Q8：孩子作業很多，每天要花多久時間複習？
**答：只需 3 ~ 5 分鐘！** RDQ 採用 Lite 輕量模式。每次對話僅 1~3 題，或 Web 端清完 2~3 張到期閃卡即可，絕不增加負擔。

### Q9：複習過的錯題，多久會再出現一次？
**答：由艾賓浩斯 Leitner Box 記憶演算法自動精算！** 答對一次升級到 Box 2（3 天後再出現）；答錯卡關退回 Box 2 防呆保護（隔天練習）。

### Q10：錯題本需要家長手動幫忙整理嗎？
**答：完全不需要！** 對話過程或上傳照片後，AI 會自動將題目、解答、失分原因 (`loss_reason`) 及 108 課綱代碼寫入 SQLite 資料庫 (`review_index.db`)。

### Q11：系統如何知道什麼時候該交棒給考前特訓？
**答：當弱點累積到一定程度或大考前夕。** 系統偵測到學生即將段考或弱點累積時，會自動提示：「弱點圖譜已準備好，建議開啟 EDS (教育決策系統) 進行決勝特訓題型派發！」

### Q12：英文、國文等需要讀課文的科目要怎麼複習最有效？
**答：強烈建議使用「第二部分：上傳課本/講義檔」功能！** 將國文課文或英文文章貼給 AI，AI 會針對課文文意、注音國字、句型進行最精確的文本內問答。
