...
ANALYSIS_BASE_DIR = os.environ.get("EDS_ANALYSIS_BASE_DIR", "exam-data/analysis")
...
def extract_official_pass_rates():
    ...
    pdf_files = sorted(glob.glob(os.path.join(
        ANALYSIS_BASE_DIR, '**', '*.pdf'), recursive=True))
...
def calculate_roi_weights(input_json_path, output_csv_path):
    if not os.path.exists(input_json_path):
        print(
            f"❌ 找不到中間檔 `{input_json_path}`，請先執行 `map_exam_to_matrix.py`。", file=sys.stderr)
        sys.exit(1)
    ...
    if default_pass_count > 0:
        print(
            f"ℹ️ 防呆提醒：共有 {default_pass_count} 筆題目無官方通過率，已採用預設值 0.5 (難度 0.5)。")
    else:
    ...
    if max_raw == min_raw:
        ...
        grouped['roi_weight'] = 0.1 + \
            (grouped['raw_roi'] - min_raw) / (max_raw - min_raw) * (1.0 - 0.1)
